...
⚠️ **Важно**: `UPLOADS_DIR` и `GENERATED_DIR` тоже должны указывать ВНУТРЬ этого
же Volume (например `/data/uploads` и `/data/generated`), а не оставаться
значениями по умолчанию (`./uploads`, `./generated`) — иначе только база данных
переживёт передеплой, а сами картинки будут стираться при каждом деплое, и в
базе останутся ссылки на несуществующие файлы (ошибка вида `ENOENT: no such
file or directory` при попытке скачать).
...
## Реальная ИИ-генерация фона под товар

Раньше "генерация карточки" была условной заглушкой (просто цветной градиент
под фото как есть). Теперь это настоящий двухэтапный ИИ-конвейер:

1. **Вырезание товара с фона** — через remove.bg или Picsart
   (`PHOTO_API_PROVIDER=removebg|picsart` + `PHOTO_API_KEY` в `.env`).
   Возвращает PNG с прозрачным фоном (альфа-канал).
2. **Генерация нового фона** — через OpenAI Images API, режим `/v1/images/edits`
   (модель `gpt-image-1`). На вход идёт вырезанный товар с прозрачным фоном —
   ИИ дорисовывает вокруг него студийный фон по текстовому промпту, сам товар
   не трогает (OpenAI использует альфа-канал входного изображения как маску
   автоматически, если явную маску не передавать — на этом и построена вся
   механика). Включается через `AI_BACKGROUND_PROVIDER=openai` + `OPENAI_API_KEY`.

Фон генерируется **один раз на загруженное фото** (не по разу на каждую
площадку — так быстрее и дешевле), а дальше просто по-разному кадрируется
под размер каждой площадки (`backend/src/photoProcessing.js`,
`renderCardFromAiPhoto`).

Если что-то из этого не настроено (нет ключей, или вырезание фона отключено
и настоящей прозрачности нет) — код автоматически откатывается на прежний
запасной вариант с плоским градиентом, без ошибок и без единого лишнего
API-вызова. Это проверено: с пустыми `.env` всё работает как раньше "из коробки".

Промпт для фона можно переопределить в `.env` (`AI_BACKGROUND_PROMPT`) —
например, задать другое настроение под конкретную нишу товаров (одежда,
электроника, косметика и т.д.).

⚠️ **Учитывайте время ответа**: генерация изображения через OpenAI занимает
заметно дольше, чем вырезание фона (может быть от нескольких секунд до
десятков секунд). Весь эндпоинт `POST /api/upload` сейчас синхронный — если
у вашего хостинга/прокси есть жёсткий таймаут на HTTP-запрос короче этого
времени, запрос может оборваться до завершения генерации. Если столкнётесь
с этим на практике — имеет смысл переделать генерацию в фоновую задачу с
поллингом статуса (аналогично тому, как уже устроена проверка оплаты).
