...
export async function applyWatermarkToVariants(variants, orderId) {
  ...
  for (const variant of variants) {
    // Размер шрифта — доля от меньшей стороны карточки. Дошли до 90% + очень
    // плотная сетка (шаг МЕНЬШЕ буквы) — на реальной карточке это оказалось
    // перебором: товар почти полностью тонет под красным, а ведь смысл
...
function buildWatermarkTiles(width, height, fontSize) {
  const yStep = Math.round(fontSize * 1.4);
  const xStep = Math.round(fontSize * 2.0);
