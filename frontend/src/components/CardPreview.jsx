...
export default function CardPreview({ t, previewUrls, productCopy, onPay, onBack }) {
  ...
  return (
    <div className="mx-auto flex w-full max-w-sm flex-col gap-4">
      ...
      <div className="rounded-3xl border border-white/10 bg-white/[0.03] p-5 shadow-xl backdrop-blur">
        <h2 className="text-lg font-bold tracking-tight">{t.previewTitle}</h2>
        <p className="mt-1 text-sm text-tg-hint">{t.previewSubtitle}</p>
      </div>

      <div className="grid grid-cols-2 gap-3">
        {previewUrls.map((url, index) => (
          <div key={url} className="overflow-hidden rounded-2xl border border-white/10 bg-white/[0.03] shadow-lg">
            <img src={absoluteUrl(url)} alt={t.variant(index + 1)} className="w-full" />
            <div className="px-3 py-2 text-center text-xs font-medium text-tg-hint">{t.variant(index + 1)}</div>
          </div>
        ))}
      </div>
      ...
      {productCopy && (
        <div className="rounded-3xl border border-white/10 bg-white/[0.03] p-5 shadow-xl backdrop-blur">
          <h3 className="text-sm font-bold tracking-tight">{t.aiCopyTitle}</h3>
          <p className="mt-0.5 text-xs text-tg-hint">{t.aiCopySubtitle}</p>
          ...
          <div className="mt-2 flex items-start justify-between gap-2 rounded-2xl bg-white/[0.03] p-3">
            ...
            <button
              ...
              className="shrink-0 text-xs text-tg-hint underline"
            >
              {copiedField === 'description' ? t.copied : t.copy}
            </button>
          </div>
